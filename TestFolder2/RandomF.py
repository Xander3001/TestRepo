...
# change .csv file to .json file from dataframe 
df.to_json (r'Path where you want to store the exported JSON file\File Name.json')
pd.read_json(r'Path where you want to store the exported JSON file\File Name.json')
#top 100 entries
print(entries[:100])
# appending titles
titles = [entry[2] for entry in entries]
# printing first 10 entries in titles
print(titles[:10])
# function to read txt file passed from command line argument to parse and clean data
def file_read(binary_file):
    with open(binary_file,'r',encoding='utf-8') as f:  # open binary file for reading using import command argv
        text = f.read() # read text
        f.close()
    text_tokens = [ word.casefold() for word in word_tokenize(text) if word not in stopwords.words('english') and word.isalpha() ]  # store tokens
    return text_tokens  #return
