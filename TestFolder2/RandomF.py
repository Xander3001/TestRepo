...
# # Import the bestsellers dataset
# # In the descriptions column only has written
# # the first 2 books from each author
# bestdf = pd.read_csv("/Users/cristagurau/Desktop/bestsellers/cleaned_bestsellers.csv")
# bestdf["Description"][0]
# 
# # Load the necessary en_core_web_sm model from spacy table of contents that include
# # names of entities, which is useful for as text-mining
...
# word_freq = Counter(nouns)
# common_nouns = word_freq.most_common(10)
# # Spacy comes with the pipeline of thousands of rules that tokenize string and converts them into the objects
# # with variuos attributes.
# # We have applied nlp on the description of the first book.
# # This returns a document which has id,punct...
# # token.text has the name of the punctuations and words
# print(common_nouns)
# 
# 
# verbs = [ token.text for token in doc if token.is_punct !=True and token.pos_ == 'VERB']
