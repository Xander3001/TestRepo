...
# Finding most common nouns and verbs in prose
...
verbs = [ token.text for token in doc if token.is_punct !=True and token.pos_ == 'VERB']
# Finding 10 most common verbs in prose
print(Counter(verbs).most_common(10))



# Redacting names from prose
def sanitize_names(text):
    doc = nlp(text)
    redacted_sentences = []
    for ent in doc.ents:
...
# Redacting locations from prose
def sanitize_locations(text):
    doc = nlp(text)
    redacted_sentences = []
    for ent in doc.ents:
...
# Define function to retrive top ndsiplay topics
